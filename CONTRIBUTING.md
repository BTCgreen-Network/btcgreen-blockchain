...
In the event an emergency fix is required for the release version of BTCgreen, members of the BTCgreen organization will create a feature branch from the current release branch `latest`.
...
sh install.sh -d
. ./activate
black . && isort benchmarks build_scripts btcgreen tests tools *.py && mypy && flake8 benchmarks build_scripts btcgreen tests tools *.py && pylint benchmarks build_scripts btcgreen tests tools *.py
...
The current official testnet is testnet10. Look at `btcgreen/util/initial_config.yaml` to see the configuration parameters
