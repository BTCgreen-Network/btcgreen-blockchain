...
If you want to learn more about this project, read the [wiki](https://github.com/BTCgreen-Network/btcgreen-blockchain/wiki), or check out the [green paper](https://www.btcgreen.com/assets/BTCgreenGreenPaper.pdf).
...
Members of the BTCgreen organization may create feature branches from the `main` branch.

In the event an emergency fix is required for the release version of BTCgreen, members of the BTCgreen organization will create a feature branch from the current release branch `1.0.0`.
...
4. BTCgreen Network's nodes on the public testnet are running the latest version `x.y.z`.
...
. ./activate
pip install ".[dev]"
black btcgreen tests && mypy btcgreen tests && flake8 btcgreen tests
...
7. Preferences > Settings > mypy > Targets: set to `./btcgreen` and `./tests`
...
Prior to the release of `1.1.0` there will be two running test nets. `testnet` and `transaction-beta-testnet`. The `transaction-beta-testnet` testnet will be a beta of the pending 1.1 release, which will enable transactions on the btcgreen blockchain.
...
To propose changes for the production releases of BTCgreen, please make a pull request to the latest release branch.
