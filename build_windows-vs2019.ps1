...
Write-Output "   ---"
Copy-Item "dist\daemon" -Destination "..\btcgreen-blockchain-gui\" -Recurse -Force
...
If ($env:HAS_SECRET) {
   ...
   Write-Output "   ---"
   signtool.exe timestamp /v /t http://timestamp.comodoca.com/ .\release-builds\windows-installer\BTCgreen-$packageVersion.exe
