...
from btcgreen.clvm.spend_sim import SimClient, SpendSim
...
from btcgreen.util.config import create_default_btcgreen_config, lock_and_load_config
from btcgreen.util.ints import uint16
from btcgreen.simulator.setup_services import setup_daemon, setup_introducer, setup_timelord
from btcgreen.util.task_timing import (
...
from btcgreen.simulator.setup_nodes import (
...
from btcgreen.simulator.wallet_tools import WalletTool
from tests.util.wallet_is_synced import wallet_is_synced

multiprocessing.set_start_method("spawn")

from pathlib import Path
from btcgreen.util.keyring_wrapper import KeyringWrapper
from btcgreen.simulator.block_tools import BlockTools, test_constants, create_block_tools, create_block_tools_async
from btcgreen.simulator.keyring import TempKeyring
from btcgreen.simulator.setup_nodes import setup_farmer_multi_harvester
...
# NOTE:
#       Instantiating the bt fixture results in an attempt to create the btcgreen root directory
...
async def empty_blockchain(request):
    ...
    from btcgreen.simulator.setup_nodes import test_constants
...
async def wallet_node_sim_and_wallet() -> AsyncIterator[
    Tuple[List[Union[FullNodeAPI, FullNodeSimulator]], List[Tuple[Wallet, BTCgreenServer]], BlockTools],
]:
...
@pytest_asyncio.fixture(scope="function")
async def two_wallet_nodes_custom_spam_filtering(spam_filter_after_n_txs, xbtc_spam_amount):
    async for _ in setup_simulators_and_wallets(1, 2, {}, spam_filter_after_n_txs, xbtc_spam_amount):
        yield _
...
@pytest_asyncio.fixture(scope="function")
async def one_node_one_block() -> AsyncIterator[Tuple[Union[FullNodeAPI, FullNodeSimulator], BTCgreenServer, BlockTools]]:
...
@pytest.fixture(scope="function")
def tmp_btcgreen_root(tmp_path):
    """
    Create a temp directory and populate it with an empty btcgreen_root directory.
    """
    path: Path = tmp_path / "btcgreen_root"
    path.mkdir(parents=True, exist_ok=True)
    return path


@pytest.fixture(scope="function")
def root_path_populated_with_config(tmp_btcgreen_root) -> Path:
    """
    Create a temp btcgreen_root directory and populate it with a default config.yaml.
    Returns the btcgreen_root path.
    """
    root_path: Path = tmp_btcgreen_root
...
@pytest.fixture(name="btcgreen_root", scope="function")
def btcgreen_root_fixture(tmp_path: Path, scripts_path: Path) -> BTCgreenRoot:
    root = BTCgreenRoot(path=tmp_path.joinpath("btcgreen_root"), scripts_path=scripts_path)
    root.run(args=["init"])
