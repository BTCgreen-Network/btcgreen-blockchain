...
from btcgreen.data_layer.data_layer_util import NodeType, Status
from btcgreen.data_layer.data_store import DataStore
from btcgreen.types.blockchain_format.tree_hash import bytes32
...
@pytest.fixture(name="btcgreen_daemon", scope="function")
def btcgreen_daemon_fixture(btcgreen_root: BTCgreenRoot) -> Iterator[None]:
    with closing_btcgreen_root_popen(btcgreen_root=btcgreen_root, args=[sys.executable, "-m", "btcgreen.daemon.server"]):
...
@pytest.fixture(name="btcgreen_data", scope="function")
def btcgreen_data_fixture(btcgreen_root: BTCgreenRoot, btcgreen_daemon: None, scripts_path: pathlib.Path) -> Iterator[None]:
    with closing_btcgreen_root_popen(btcgreen_root=btcgreen_root, args=[os.fspath(scripts_path.joinpath("btcgreen_data_layer"))]):
        # TODO: this is not pretty as a hard coded time
        # let it settle
        time.sleep(5)
        yield
