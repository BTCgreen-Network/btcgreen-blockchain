...
from btcgreen.protocols import farmer_protocol
from btcgreen.protocols.harvester_protocol import Plot
from btcgreen.rpc.farmer_rpc_api import (
    FilterItem,
...
from btcgreen.simulator.block_tools import get_plot_dir
...
from btcgreen.util.config import load_config, lock_and_load_config, save_config
from btcgreen.util.hash import std_hash
from btcgreen.util.ints import uint8, uint32, uint64
from btcgreen.util.misc import get_list_or_len
...
async def test_farmer_reward_target_endpoints(harvester_farmer_environment):
    ...
    await farmer_rpc_client.set_reward_targets(encode_puzzle_hash(new_ph, "xbtc"), encode_puzzle_hash(new_ph_2, "xbtc"))
    ...
    await farmer_rpc_client.set_reward_targets(
        encode_puzzle_hash(observer_farmer, "xbtc"), encode_puzzle_hash(observer_pool, "xbtc")
    )
    ...
    assert config["farmer"]["xbtc_target_address"] == encode_puzzle_hash(observer_farmer, "xbtc")
    assert config["pool"]["xbtc_target_address"] == encode_puzzle_hash(observer_pool, "xbtc")

    new_ph_2_encoded = encode_puzzle_hash(new_ph_2, "xbtc")
