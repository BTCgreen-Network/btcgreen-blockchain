...
async def test_nft_mint_from_did(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    expected_xbtc_bal = funds - fee - mint_total - 1
    await time_out_assert(30, wallet_0.get_confirmed_balance, expected_xbtc_bal)
...
async def test_nft_mint_from_did_rpc(two_wallet_nodes: Any, trusted: Any, self_hostname: str) -> None:
    ...
    try:
        ...
        target_list = [encode_puzzle_hash((ph_taker), "xbtc") for x in range(n)]
        royalty_address = encode_puzzle_hash(bytes32(token_bytes(32)), "xbtc")
        royalty_percentage = 300
        fee = 100
        required_amount = n + (fee * n)
        xbtc_coins = await client.select_coins(amount=required_amount, wallet_id=wallet_maker.id())
        funding_coin = xbtc_coins[0]
        assert funding_coin.amount >= required_amount
        funding_coin_dict = xbtc_coins[0].to_json_dict()
        chunk = 5
        ...
        for i in range(0, n, chunk):
            resp: Dict[str, Any] = await client.nft_mint_bulk(
                ...
                mint_total=n,
                xbtc_coins=[next_coin.to_json_dict()],
                xbtc_change_target=funding_coin_dict["puzzle_hash"],
            ...
            spends.append(sb)
            xbtc_adds = [c for c in sb.additions() if c.puzzle_hash == funding_coin.puzzle_hash]
            assert len(xbtc_adds) == 1
            next_coin = xbtc_adds[0]
...
@pytest.mark.asyncio
async def test_nft_mint_from_did_multiple_xbtc(two_wallet_nodes: Any, trusted: Any) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
    ...
    # Grab two coins for testing that we can create a bulk minting with more than 1 xbtc coin
    xbtc_coins_1 = await wallet_maker.select_coins(amount=10000)
    xbtc_coins_2 = await wallet_maker.select_coins(amount=10000, exclude=xbtc_coins_1)
    xbtc_coins = xbtc_coins_1.union(xbtc_coins_2)
    ...
    # confirm that the spend uses the right amount of xbtc
    expected_xbtc_bal = funds - fee - mint_total - 1
    await time_out_assert(30, wallet_maker.get_confirmed_balance, expected_xbtc_bal)
...
@pytest.mark.asyncio
async def test_nft_mint_from_xbtc(two_wallet_nodes: Any, trusted: Any) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
    ...
    sb = await nft_wallet_maker.mint_from_xbtc(
        metadata_list, target_list=target_list, mint_number_start=1, mint_total=mint_total, fee=fee
    )
...
@pytest.mark.asyncio
async def test_nft_mint_from_xbtc_rpc(two_wallet_nodes: Any, trusted: Any, self_hostname: str) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    bt = full_node_api.bt
    full_node_server = full_node_api.server
...
@pytest.mark.asyncio
async def test_nft_mint_from_xbtc_multiple_xbtc(two_wallet_nodes: Any, trusted: Any) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
