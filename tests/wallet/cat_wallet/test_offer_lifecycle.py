...
from btcgreen.wallet.cat_wallet.cat_utils import (
...
from btcgreen.wallet.trading.offer import Offer, NotarizedPayment
...
# This method takes a dictionary of strings mapping to amounts and generates the appropriate CAT/XBTC coins
...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            btcgreen_coins: List[Coin] = all_coins[None]
            ...
            # Create an XBTC Offer for RED
            btcgreen_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            btcgreen_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                btcgreen_requested_payments, btcgreen_coins
            )
            btcgreen_announcements: List[Announcement] = Offer.calculate_announcements(btcgreen_requested_payments, driver_dict)
            btcgreen_secured_bundle: SpendBundle = generate_secure_bundle(btcgreen_coins, btcgreen_announcements, 1000)
            btcgreen_offer = Offer(btcgreen_requested_payments, btcgreen_secured_bundle, driver_dict)
            assert not btcgreen_offer.is_valid()

            # Create a RED Offer for XBTC
            ...
            new_offer = Offer.aggregate([btcgreen_offer, red_offer, red_offer_2])
            ...
            # Create yet another offer of BLUE for XBTC and RED
            ...
            assert new_offer.summary() == (
                ...
                {"xbtc": 900, str_to_tail_hash("red").hex(): 350},
