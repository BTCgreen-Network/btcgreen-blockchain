...
from btcgreen.clvm.spend_sim import SpendSim, SimClient
...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            btcgreen_announcements: List[Announcement] = Offer.calculate_announcements(btcgreen_requested_payments)
            btcgreen_secured_bundle: SpendBundle = self.generate_secure_bundle(btcgreen_coins, btcgreen_announcements, 1000)
            btcgreen_offer = Offer(btcgreen_requested_payments, btcgreen_secured_bundle)
            ...
            new_offer = Offer.aggregate([btcgreen_offer, red_offer])
