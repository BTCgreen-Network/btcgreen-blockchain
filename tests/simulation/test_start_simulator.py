...
from btcgreen.simulator.simulator_full_node_rpc_client import SimulatorFullNodeRpcClient
from btcgreen.simulator.simulator_test_tools import get_full_btcgreen_simulator, get_puzzle_hash_from_key
...
class TestStartSimulator:
    ...
    async def get_btcgreen_simulator(
        self, tmp_path: Path, empty_keyring: Keychain
    ) -> AsyncGenerator[Tuple[FullNodeSimulator, Path, Dict[str, Any], str, int, Keychain], None]:
        async for simulator_args in get_full_btcgreen_simulator(btcgreen_root=tmp_path, keychain=empty_keyring):
            yield simulator_args

    @pytest.mark.asyncio
    async def test_start_simulator(
        self, get_btcgreen_simulator: Tuple[FullNodeSimulator, Path, Dict[str, Any], str, int, Keychain]
    ) -> None:
        simulator, root_path, config, mnemonic, fingerprint, keychain = get_btcgreen_simulator
