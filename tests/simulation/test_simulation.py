...
from btcgreen.simulator.block_tools import BlockTools, create_block_tools_async
from btcgreen.simulator.full_node_simulator import FullNodeSimulator
from btcgreen.simulator.simulator_protocol import FarmNewBlockProtocol, GetAllCoinsProtocol, ReorgProtocol
...
from btcgreen.simulator.setup_nodes import SimulatorsAndWallets, setup_simulators_and_wallets, setup_full_system
from btcgreen.simulator.setup_services import setup_full_node
...
class TestSimulation:
    ...
    async def test_simulation_1(self, simulation, extra_node, self_hostname):
        ...
        server1: BTCgreenServer = node1.full_node.server
        ...
        server3: BTCgreenServer = node3.full_node.server
    ...
    @pytest.mark.parametrize(
        ...
        argvalues=[
            [0, 0],
            [1, 2],
            [(2 * units["btcgreen"]) - 1, 2],
            [2 * units["btcgreen"], 2],
            [(2 * units["btcgreen"]) + 1, 4],
            [3 * units["btcgreen"], 4],
            [10 * units["btcgreen"], 10],
        ],
    )
