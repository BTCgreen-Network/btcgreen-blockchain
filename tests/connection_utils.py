...
from btcgreen.protocols.shared_protocol import capabilities, protocol_version
...
from btcgreen.server.ssl_context import btcgreen_ssl_ca_paths
...
async def disconnect_all(server: BTCgreenServer) -> None:
...
async def disconnect_all_and_reconnect(server: BTCgreenServer, reconnect_to: BTCgreenServer, self_hostname: str) -> bool:
...
async def add_dummy_connection(
    server: BTCgreenServer, self_hostname: str, dummy_port: int, type: NodeType = NodeType.FULL_NODE
) -> Tuple[asyncio.Queue, bytes32]:
    timeout = aiohttp.ClientTimeout(total=10)
    session = aiohttp.ClientSession(timeout=timeout)
    incoming_queue: asyncio.Queue = asyncio.Queue()
    config = load_config(server.root_path, "config.yaml")
    btcgreen_ca_crt_path, btcgreen_ca_key_path = btcgreen_ssl_ca_paths(server.root_path, config)
    dummy_crt_path = server.root_path / "dummy.crt"
    dummy_key_path = server.root_path / "dummy.key"
    generate_ca_signed_cert(
        btcgreen_ca_crt_path.read_bytes(), btcgreen_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(btcgreen_ca_crt_path, btcgreen_ca_key_path, dummy_crt_path, dummy_key_path)
    ...
    wsc = WSBTCgreenConnection.create(
        type,
        ws,
        server._port,
        log,
...
async def connect_and_get_peer(server_1: BTCgreenServer, server_2: BTCgreenServer, self_hostname: str) -> WSBTCgreenConnection:
    """
