...
from btcgreen.cmds.init_funcs import create_all_ssl, create_default_btcgreen_config
from btcgreen.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
...
from btcgreen.plotting.create_plots import create_plots, PlotKeys
...
from btcgreen.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
...
from btcgreen.plotting.util import PlotsRefreshParameter, PlotRefreshResult, PlotRefreshEvents, parse_plot_info
    ...
    async def setup_keys(self):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="btcgreen-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `btcgreen generate keys`")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("BTCGREEN_ROOT", "~/.btcgreen/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
