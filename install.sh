...
install_python3_and_sqlite3_from_source_with_yum() {
  ...
  cd "$TMP_PATH"
  # Install sqlite>=3.37
  # yum install sqlite-devel brings sqlite3.7 which is not compatible with btcgreen
  ...
  # yum install python3 brings Python3.6 which is not supported by btcgreen
  cd ..
...
if [ "$PYTHON_MAJOR_VER" -ne "3" ] || [ "$PYTHON_MINOR_VER" -lt "7" ] || [ "$PYTHON_MINOR_VER" -ge "11" ]; then
  echo "BTCgreen requires Python version >= 3.7 and  < 3.11.0" >&2
...
echo "https://keybase.io/team/btcgreen_network.public"
echo ""
