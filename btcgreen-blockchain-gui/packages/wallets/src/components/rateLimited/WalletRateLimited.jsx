import { AlertDialog, Card, Flex, btcgreenToMojo, mojoToBTCgreenLocaleString } from '@btcgreen/core';
...
function IncompleteCard(props) {
  ...
  function submit() {
    ...
    const btcgreenper_input = ip_parsed.limit;
    ...
    const btcgreenper_value = Number.parseInt(Number(btcgreenper_input));
    const origin_parsed = JSON.parse(origin_input);
    dispatch(rl_set_user_info_action(id, interval_value, btcgreenper_value, origin_parsed, admin_pubkey_input));
  }
      ...
      <Card title={<Trans>Rate Limited Info</Trans>}>
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>Spending Limit (btcgreen per interval): {mojoToBTCgreenLocaleString(limit)}</Trans>
              </Typography>
            </Box>
          </Box>
        </Grid>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToBTCgreenLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
function SendCard(props) {
  ...
  function send() {
    ...
    const amount = btcgreenToMojo(amount_input.value);
    const fee = btcgreenToMojo(fee_input.value);
