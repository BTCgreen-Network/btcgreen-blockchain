...
import { TransactionType } from '@btcgreen/api';
import type { Transaction } from '@btcgreen/api';
import { mojoToBTCgreen, blockHeightToTimestamp } from '@btcgreen/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, mojoToBTCgreen(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToBTCgreen(balance).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, mojoToBTCgreen(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToBTCgreen(start).toString, // bignumber is not supported by react
    });
  });
