import { TransactionType, WalletType } from '@btcgreen/api';
import type { Transaction } from '@btcgreen/api';
import { useGetWalletBalanceQuery } from '@btcgreen/api-react';
import { mojoToBTCgreen, mojoToCAT, blockHeightToTimestamp } from '@btcgreen/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToBTCgreen(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: (walletType === WalletType.CAT ? mojoToCAT(balance) : mojoToBTCgreen(balance)).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToBTCgreen(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: walletType === WalletType.CAT ? mojoToCAT(start) : mojoToBTCgreen(start).toString(), // bignumber is not supported by react
    });
  });
