import { AlertDialog, Card, Flex, Loading, Dropzone, mojoToBTCgreenLocaleString } from '@btcgreen/core';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">{mojoToBTCgreenLocaleString(props.balance)} TXBTC</Typography>
        </Box>
      </Box>
    </Grid>
  );
...
function CreateAttest(props) {
  ...
  function createAttestPacket() {
    ...
    if (address.substring(0, 12) === 'btcgreen_addr://') {
...
function CashoutCard(props) {
  ...
  function cashout() {
    ...
    if (puzzlehash.slice(0, 12) === 'btcgreen_addr://') {
