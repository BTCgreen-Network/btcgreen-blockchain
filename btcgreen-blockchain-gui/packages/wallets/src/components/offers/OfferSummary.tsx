...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount = makerEntries[0][0].toLowerCase() === 'xbtc' ? Number(mojoToBTCgreen(makerEntries[0][1])) : Number(mojoToCAT(makerEntries[0][1]));
  const takerAmount = takerEntries[0][0].toLowerCase() === 'xbtc' ? Number(mojoToBTCgreen(takerEntries[0][1])) : Number(mojoToCAT(takerEntries[0][1]));
