import { WalletType, TransactionType, toBech32m } from '@btcgreen/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
    ...
    field: (row: Row, metadata) => {
      ...
      return (
        <>
          ...
          &nbsp;
          <strong>
            <FormatLargeNumber value={type === WalletType.CAT ? mojoToCAT(row.amount) : mojoToBTCgreen(row.amount)} />
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>
          <FormatLargeNumber value={mojoToBTCgreen(row.feeAmount)} />
        </strong>
        &nbsp;
        {metadata.feeUnit}
      </>
