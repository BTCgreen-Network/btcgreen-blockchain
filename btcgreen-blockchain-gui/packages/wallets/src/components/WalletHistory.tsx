...
import { Card, CardKeyValue, CopyToClipboard, Flex, Loading, StateColor, TableControlled, toBech32m, useCurrencyCode, mojoToBTCgreenLocaleString, mojoToCATLocaleString } from '@btcgreen/core';
import { useGetOfferRecordMutation, useGetSyncStatusQuery } from '@btcgreen/api-react';
import styled from 'styled-components';
import type { Row } from '@btcgreen/core';
import { WalletType, TransactionType } from '@btcgreen/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
        ...
        <>
          ...
          <strong>
            {type === WalletType.CAT
              ...
              : mojoToBTCgreenLocaleString(row.amount)}
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>{mojoToBTCgreenLocaleString(row.feeAmount)}</strong>
        &nbsp;
        {metadata.feeUnit}
      </>
    ),
