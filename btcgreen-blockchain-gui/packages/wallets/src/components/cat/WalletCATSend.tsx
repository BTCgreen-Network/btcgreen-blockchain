import { SyncingStatus, toBech32m } from '@btcgreen/api';
import { useSpendCATMutation, useFarmBlockMutation } from '@btcgreen/api-react';
import {
  AdvancedOptions,
  Button,
  EstimatedFee,
...
export default function WalletCATSend(props: Props) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      throw new Error(t`Cannot send btcgreen to coloured address. Please enter a btcgreen address.`);
    }
    ...
    const feeValue = btcgreenToMojo(fee);
