...
import { useGetWalletsQuery, useGetStrayCatsQuery, useGetCatListQuery, useAddCATTokenMutation } from '@btcgreen/api-react';
import { useShowError } from '@btcgreen/core';
  ...
  const list = useMemo(() => {
    ...
    let tokens = [
      ...baseWallets.map((wallet: Wallet) => ({
        ...
        name: wallet.type === WalletType.STANDARD_WALLET ? 'BTCgreen' : wallet.meta?.name ?? wallet.name,
      })),
