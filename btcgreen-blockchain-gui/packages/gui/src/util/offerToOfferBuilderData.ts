import { mojoToCAT, mojoToBTCgreen } from '@btcgreen/core';
...
export default function offerToOfferBuilderData(offerSummary: OfferSummary): OfferBuilderData {
  ...
  Object.keys(requested).forEach((id) => {
    ...
    if (info?.type === 'CAT') {
      ...
      offeredXch.push({
        amount: mojoToBTCgreen(amount).toFixed(),
      });
    }
  });
  ...
  return {
    ...
    requested: {
      ...
      fee: [
        {
          amount: mojoToBTCgreen(fees).toFixed(),
        },
      ],
    },
  };
