...
export function getConfigRootDir(net = 'mainnet'): string {
  ...
  return 'BTCGREEN_ROOT' in process.env ? untildify(process.env.BTCGREEN_ROOT) : path.join(homedir, '.btcgreen', net);
}
...
export default async function loadConfig(net?: string): Promise<{
  ...
  try {
    ...
    const daemonPort = get(config, 'ui.daemon_port', 44276);
