...
export function getUserDataDir(): string {
  const btcgreenRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(btcgreenRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const btcgreenRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${btcgreenRootUserDataPath}`);
  app.setPath('userData', btcgreenRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(btcgreenRootUserDataPath, 'Local Storage', 'leveldb');
