...
import { useGetBlockQuery, useGetBlockRecordQuery  } from '@btcgreen/api-react'
...
export default function Block() {
  ...
  const poolReward = mojoToBTCgreen(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToBTCgreen(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const btcgreenFees = blockRecord.fees !== undefined
    ? mojoToBTCgreen(blockRecord.fees)
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.btcgreenexplorer.org/blockchain/puzzlehash/${blockRecord.farmerPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.btcgreenexplorer.org/blockchain/puzzlehash/${blockRecord.poolPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: btcgreenFees ? `${btcgreenFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <>
      ...
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the BTCgreen blockchain
            </Trans>
          </Back>
        }
        action={
