import { usePwAbsorbRewardsMutation, useGetPlotNFTsQuery, useGetCurrentAddressQuery } from '@btcgreen/api-react';
import {
  UnitFormat,
  CardStep,
  ButtonLoading,
...
export default function PlotNFTAbsorbRewards(props: Props) {
  ...
  async function handleSubmit(data: SubmitData) {
    try {
      ...
      const { fee } = data;
      const feeMojos = btcgreenToMojo(fee);
        ...
        <Flex flexDirection="column" gap={3}>
          ...
          >
            <Typography variant="subtitle1">
              <Trans>
                You will recieve{' '}
                <UnitFormat value={mojoToBTCgreenLocaleString(balance)} display="inline" state={State.SUCCESS} /> to{' '}
                {address}
              </Trans>
            </Typography>
