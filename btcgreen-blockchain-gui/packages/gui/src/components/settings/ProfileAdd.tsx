import { useCreateNewWalletMutation, useGetWalletBalanceQuery } from '@btcgreen/api-react';
import { ButtonLoading, btcgreenToMojo, EstimatedFee, Flex, Form, mojoToBTCgreenLocaleString } from '@btcgreen/core';
...
export default function ProfileAdd() {
  ...
  async function handleSubmit(data: CreateProfileData) {
    ...
    const walletId = await createProfile({
      ...
      options: { did_type: 'new', backup_dids: [], num_of_backup_ids_needed: '0', amount: 1, fee: btcgreenToMojo(fee) },
    }).unwrap();

    navigate(`/dashboard/settings/profiles/${walletId}`);
  }

  const standardBalance = mojoToBTCgreenLocaleString(balance?.confirmedWalletBalance);
        ...
        <StyledCard>
          ...
          <Flex flexDirection="column" gap={2.5} paddingBottom={3}>
            <Typography variant="caption">
              <Trans>Balance: {standardBalance} XBTC</Trans>
            </Typography>
          </Flex>
          <Flex flexDirection="column" gap={2.5} paddingBottom={1}>
            <EstimatedFee
          ...
          <Flex flexDirection="column" gap={2.5} paddingBottom={3}>
            <Typography variant="caption">
              <Trans>Recommended: 0.000005 XBTC</Trans>
            </Typography>
          </Flex>
