import { type OfferSummaryRecord } from '@btcgreen/api';
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToBTCgreen, mojoToCAT } from '@btcgreen/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['xbtc', 'txbtc'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToBTCgreen(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['xbtc', 'txbtc'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToBTCgreen(takerEntries[0][1])
