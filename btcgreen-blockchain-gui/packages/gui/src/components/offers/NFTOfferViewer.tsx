...
import { useCheckOfferValidityMutation, useGetNFTInfoQuery, useGetNFTWallets } from '@btcgreen/api-react';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const assetIdsToTypes: { [key: string]: OfferAsset | undefined }[] = useMemo(
    () =>
      Object.keys(summaryData).map((key) => {
        ...
        if (['xbtc', 'txbtc'].includes(key.toLowerCase())) {
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.BTCGREEN: // fall-through
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const xbtcMakerFee = mojoToBTCgreen(makerFee);

    return {
      ...calculateNFTRoyalties(
        amount,
        parseFloat(xbtcMakerFee),
  ...
  const overrideNFTSellerAmount =
    exchangeType === NFTOfferExchangeType.TokenForNFT
      ? assetType === OfferAsset.BTCGREEN
        ? btcgreenToMojo(nftSaleInfo?.nftSellerNetAmount ?? 0)
