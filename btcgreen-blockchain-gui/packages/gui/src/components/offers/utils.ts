...
export function determineNFTOfferExchangeType(
  ...
  return nftOffered
    ? NFTOfferExchangeType.NFTForXBTC
    : NFTOfferExchangeType.XBTCForNFT;
}
...
export function getNFTPriceWithoutRoyalties(
  ...
  // NFTs can only be exchanged for XBTC currently
  const amountInMojos = offerAssetAmountForAssetId('xbtc', summary);
  if (amountInMojos === undefined) {
    return undefined;
  }
  return mojoToBTCgreen(amountInMojos).toNumber();
}
