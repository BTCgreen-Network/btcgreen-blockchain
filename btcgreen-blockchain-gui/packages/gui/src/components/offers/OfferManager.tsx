import { OfferSummaryRecord, OfferTradeRecord } from '@btcgreen/api';
import { useCancelOfferMutation, useGetOfferDataMutation, useGetWalletsQuery } from '@btcgreen/api-react';
import {
  Button,
  ButtonLoading,
  Card,
...
function ConfirmOfferCancellation(props: ConfirmOfferCancellationProps) {
  ...
  async function handleConfirm() {
    const { fee: xbtcFee } = methods.getValues();

    const fee = cancelWithTransaction ? btcgreenToMojo(xbtcFee) : new BigNumber(0);
...
function OfferList(props: OfferListProps) {
  ...
  const testnet = useCurrencyCode() === 'TXBTC';
                  ...
                  <Flex flexDirection="column" gap={4} height="100%">
                    <Flex flexDirection="column" gap={2} flexGrow={1}>
                      ...
                      <Typography variant="body1" color="textSecondary">
                        <Trans>
                          Create a file that you can use to trade XBTC, BTCgreen Asset Tokens, or NFTs with no counter-party
                          risk.
                        </Trans>
                      </Typography>
                    </Flex>
