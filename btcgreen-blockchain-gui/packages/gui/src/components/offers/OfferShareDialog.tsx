...
const log = debug('btcgreen-gui:offers');
...
enum HashgreenErrorCodes {
  ...
  MARKET_NOT_FOUND = 50029, // Pairing doesn't exist e.g. XBTC/RandoCoin
...
async function postToSpacescan(offerData: string, testnet: boolean): Promise<string> {
  ...
  const requestOptions = {
    ...
    port: 443,
    path: `/api/offer/upload?coin=${testnet ? 'txbtc' : 'xbtc'}&version=1`,
  };
  const requestHeaders = {
    'Content-Type': 'application/json',
  };
  ...
  return `https://www.spacescan.io/${testnet ? 'txbtc' : 'xbtc'}/offer/${id}`;
}
...
const KeybaseTeamName = 'btcgreen_offers';
...
function OfferShareKeybaseDialog(props: OfferShareServiceDialogProps) {
  ...
  const teamName = testnet ? 'testxbtcoffersdev' : KeybaseTeamName;
