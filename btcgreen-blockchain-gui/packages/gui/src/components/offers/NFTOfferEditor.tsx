...
import type { NFTInfo, Wallet } from '@btcgreen/api';
import {
...
/*        An NFT offer consists of a single NFT being offered for XBTC         */
/* ========================================================================== */
...
function NFTOfferConditionalsPanel(props: NFTOfferConditionalsPanelProps) {
  ...
  const amount = methods.watch('xbtcAmount');
  ...
  const offerElem =
    tab === NFTOfferExchangeType.NFTForXBTC ? nftElem : amountElem;
  const takerElem =
    tab === NFTOfferExchangeType.NFTForXBTC ? amountElem : nftElem;
  ...
  function handleAmountChange(amount: string) {
    methods.setValue('xbtcAmount', amount);
  }
  ...
  return (
    ...
    >
      ...
      >
        <Tab
          value={NFTOfferExchangeType.NFTForXBTC}
          label={<Trans>NFT for XBTC</Trans>}
          disabled={disabled}
        />
        <Tab
          value={NFTOfferExchangeType.XBTCForNFT}
          label={<Trans>XBTC for NFT</Trans>}
          disabled={disabled}
        />
      </Tabs>
      <Grid container>
        <Flex flexDirection="column" flexGrow={1} gap={3}>
          ...
          {tab === NFTOfferExchangeType.XBTCForNFT && (
            <Flex flexDirection="column" gap={2}>
              ...
              <>
                <Flex flexDirection="column" gap={0.5}>
                  <Typography variant="body1" color="textSecondary">
                    {tab === NFTOfferExchangeType.NFTForXBTC ? (
...
/* ========================================================================== */
/*                              NFT Offer Editor                              */
/*             Currently only supports a single NFT <--> XBTC offer            */
/* ========================================================================== */
...
function buildOfferRequest(
  ...
  const baseMojoAmount: BigNumber = btcgreenToMojo(xbtcAmount);
  const mojoAmount =
    exchangeType === NFTOfferExchangeType.NFTForXBTC
      ? baseMojoAmount
      : baseMojoAmount.negated();
  const feeMojoAmount = btcgreenToMojo(fee);
  const nftAmount = exchangeType === NFTOfferExchangeType.NFTForXBTC ? -1 : 1;
  const xbtcWalletId = 1;
  ...
  return [
    {
      ...
      [xbtcWalletId]: mojoAmount,
    },
    exchangeType === NFTOfferExchangeType.XBTCForNFT ? driverDict : undefined,
    // driverDict,
    feeMojoAmount,
  ];
}
...
export default function NFTOfferEditor(props: NFTOfferEditorProps) {
  ...
  function validateFormData(
    ...
    const { exchangeType, nftId, xbtcAmount, fee } = unvalidatedFormData;
    ...
    } else if (!xbtcAmount || xbtcAmount === '0') {
  ...
  async function handleSubmit(unvalidatedFormData: NFTOfferEditorFormData) {
    ...
    const { exchangeType, launcherId, xbtcAmount, fee } = formData;

    if (exchangeType === NFTOfferExchangeType.NFTForXBTC) {
      const haveNFT =
