import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@btcgreen/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@btcgreen/api';
import { useCalculateRoyaltiesForNFTsQuery, useGetNFTsByNFTIDsQuery, useGetWalletsQuery } from '@btcgreen/api-react';
import { catToMojo, btcgreenToMojo } from '@btcgreen/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedXBTC = useWatch({
    name: 'requested.xbtc',
  });

  const offeredXBTC = useWatch({
    name: 'offered.xbtc',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedXBTC ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('xbtc', btcgreenToMojo(amount))),
    ...(requestedTokens ?? [])
