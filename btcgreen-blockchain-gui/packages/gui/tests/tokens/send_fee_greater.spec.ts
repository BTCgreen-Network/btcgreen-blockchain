...
test('Confirm that User cannot send a TXBTC fee greater then in Wallet', async () => {
  ...
  await page.locator('text=Fee *TXBTC >> input[type="text"]').fill('10');
  ...
  await expect(page.locator('text=Pending Change0 TXBTC')).toBeVisible();
});
