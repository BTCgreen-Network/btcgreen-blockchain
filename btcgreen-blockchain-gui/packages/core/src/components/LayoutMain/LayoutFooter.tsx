...
async function openFAQURL(): Promise<void> {
  try {
    ...
    await shell.openExternal('https://github.com/BTCgreen-Network/btcgreen-blockchain/wiki/FAQ');
  }
  catch (e) {
    console.error(e);
  }
...
async function openSendFeedbackURL(): Promise<void> {
  try {
    const shell: Shell = (window as any).shell;
    await shell.openExternal('https://feedback.btcgreennetwork.org/lightwallet');
  }
  catch (e) {
    console.error(e);
  }
