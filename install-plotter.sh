...
get_madmax_filename() {
  ...
  echo "${BTCGREEN_PLOT}-${MADMAX_VER}-${SUFFIX}"
}
...
if [ "$PLOTTER" = "bladebit" ]; then
  ...
  mv -f "${madmax_filename}" btcgreen_plot
  chmod 755 btcgreen_plot
  echo "Successfully installed madmax to $(pwd)/btcgreen_plot"
  ...
  mv -f "${madmax_filename}" btcgreen_plot_k34
  chmod 755 btcgreen_plot_k34
  echo "Successfully installed madmax for k34 to $(pwd)/btcgreen_plot_k34"
else
  usage
fi
