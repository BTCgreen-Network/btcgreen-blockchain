...
from btcgreen.cmds.passphrase_funcs import obtain_current_passphrase
from btcgreen.consensus.coinbase import create_puzzlehash_for_pk
from btcgreen.daemon.client import connect_to_daemon_and_validate
...
from btcgreen.util.errors import KeychainException, KeychainNotSet
from btcgreen.util.file_keyring import MAX_LABEL_LENGTH
from btcgreen.util.ints import uint32
from btcgreen.util.keychain import Keychain, bytes_to_mnemonic, generate_mnemonic, mnemonic_to_seed
...
def show_all_key_labels() -> None:
    ...
    if len(keys) == 0:
        sys.exit("No keys are present in the keychain. Generate them with 'btcgreen keys generate'")
...
def derive_sk_from_hd_path(master_sk: PrivateKey, hd_path_root: str) -> Tuple[PrivateKey, str]:
    ...
    from btcgreen.wallet.derive_keys import _derive_path, _derive_path_unhardened
...
async def migrate_keys(root_path: Path, forced: bool = False) -> bool:
    ...
    deprecation_message = (
        ...
        "You need to migrate your keyring to continue using BTCgreen.\n"
    )
