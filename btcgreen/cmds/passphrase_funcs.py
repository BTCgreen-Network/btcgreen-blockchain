...
from btcgreen.daemon.client import acquire_connection_to_daemon
from btcgreen.util.config import load_config
from btcgreen.util.errors import KeychainMaxUnlockAttempts
from btcgreen.util.keychain import Keychain, supports_os_passphrase_storage
...
from btcgreen.util.ws_message import WsRpcMessage
    ...
    try:
        if supports_os_passphrase_storage():
            ...
            print(
                ...
                "Other BTCgreen processes will be able to access your keys without prompting for your passphrase."
            )
...
def read_passphrase_from_file(passphrase_file: TextIOWrapper) -> str:
    passphrase = passphrase_file.read().rstrip(os.environ.get("BTCGREEN_PASSPHRASE_STRIP_TRAILING_CHARS", "\r\n"))
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'btcgreen passphrase set' or 'btcgreen passphrase remove' to update or remove your passphrase")
        sys.exit(1)
