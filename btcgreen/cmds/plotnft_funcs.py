...
from btcgreen.cmds.cmds_util import get_any_service_client, transaction_status_msg, transaction_submitted_msg
from btcgreen.cmds.units import units
from btcgreen.cmds.wallet_funcs import print_balance, wallet_coin_unit
from btcgreen.pools.pool_config import PoolWalletConfig, load_pool_config, update_pool_config
...
async def create(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee_mojos = uint64(int(fee * units["btcgreen"]))
    ...
    if user_input.lower() == "y" or user_input.lower() == "yes":
        try:
            ...
            print(f"Error creating plot NFT: {e}\n    Please start both farmer and wallet with:  btcgreen start -r farmer")
        return
    print("Aborting.")
