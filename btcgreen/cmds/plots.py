...
from btcgreen.plotting.util import add_plot_directory, validate_plot_size
...
def show_plots(root_path: Path):
    from btcgreen.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'btcgreen plots add -d [dir]' and remove with"
        + " 'btcgreen plots remove -d [dir]'"
        + " Scan and check plots with 'btcgreen plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'btcgreen init'")
...
def create_cmd(
    ...
    from btcgreen.plotting.create_plots import create_plots, resolve_plot_keys
...
def check_cmd(
    ...
    from btcgreen.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from btcgreen.plotting.util import remove_plot_directory
