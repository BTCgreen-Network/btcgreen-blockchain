...
@click.option(
    ...
    "-d",
    help=(
        "Show the first wallet address using non-observer derivation. Older BTCgreen versions use "
...
@keys_cmd.command("migrate", short_help="Attempt to migrate keys to the BTCgreen keyring")
@click.pass_context
...
@click.option("--prefix", "-x", help="Address prefix (xbtc for mainnet, txbtc for testnet)", default=None, type=str)
@click.pass_context
def search_cmd(
    ctx: click.Context,
