...
from btcgreen.cmds.cmds_util import transaction_submitted_msg, transaction_status_msg
from btcgreen.cmds.show import print_connections
...
from btcgreen.wallet.nft_wallet.nft_info import NFT_HRP, NFTInfo
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    final_fee = uint64(int(fee * units["btcgreen"]))
    final_amount: uint64
    final_min_coin_amount: uint128
    if typ == WalletType.STANDARD_WALLET:
        final_amount = uint64(int(amount * units["btcgreen"]))
        final_min_coin_amount = uint128(int(min_coin_amount * units["btcgreen"]))
    ...
    print(f"Do 'btcgreen wallet get_transaction -f {fingerprint} -tx 0x{tx_id}' to get status")
...
async def take_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if nft_coin_id is not None and nft_royalty_percentage > 0:
        ...
        if nft_royalty_asset_id == "btcgreen":
        ...
        nft_royalty_divisor = units["btcgreen"] if nft_royalty_asset_id == "btcgreen" else units["cat"]
...
async def get_wallet(wallet_client: WalletRpcClient, fingerprint: int = None) -> Optional[Tuple[WalletRpcClient, int]]:
    ...
    if len(fingerprints) == 0:
        print("No keys loaded. Run 'btcgreen keys generate' or import a key")
        return None
