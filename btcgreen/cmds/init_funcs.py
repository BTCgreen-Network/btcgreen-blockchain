...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("xbtc_target_address") or all_targets[-2] == config["farmer"].get(
                "xbtc_target_address"
            ):
    ...
    if "xbtc_target_address" not in config["farmer"]:
        print(
            f"Setting the xbtc destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
