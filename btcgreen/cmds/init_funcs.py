...
from btcgreen.cmds.configure import configure
...
from btcgreen.util.db_version import set_db_version
    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("xbtc_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("xbtc_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("xbtc_target_address") or all_targets[-2] == config["pool"].get(
                    "xbtc_target_address"
                ):
        ...
        if "xbtc_target_address" not in config["farmer"]:
            print(
                f"Setting the xbtc destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
        ...
        if "xbtc_target_address" not in config["pool"]:
            print(f"Setting the xbtc destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        ...
        if updated_target:
            print(
                f"To change the XBTC destination addresses, edit the `xbtc_target_address` entries in"
...
def init(
    ...
    if create_certs is not None:
        ...
        return btcgreen_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)
...
def btcgreen_init(
    ...
    *,
    ...
    """
    btcgreen_root = os.environ.get("BTCGREEN_ROOT", None)
    if btcgreen_root is not None:
        print(f"BTCGREEN_ROOT is set to {btcgreen_root}")
