...
from btcgreen.cmds.cmds_util import execute_with_wallet
from btcgreen.cmds.coins import coins_cmd
from btcgreen.cmds.plotnft import validate_fee
...
@wallet_cmd.command("send", short_help="Send btcgreen to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-a", "--amount", help="How much btcgreen to send, in XBTC", type=str, required=True)
...
@click.option("-t", "--address", help="Address to send the XBTC", type=str, required=True)
@click.option(
...
@click.option(
    "-ma",
    "--min-coin-amount",
    help="Ignore coins worth less then this much XBTC or CAT units",
    type=str,
    required=False,
    default="0",
)
@click.option(
    "-l",
    "--max-coin-amount",
    help="Ignore coins worth more then this much XBTC or CAT units",
    type=str,
    required=False,
    default="0",
)
...
def address_sign_message(wallet_rpc_port: Optional[int], fingerprint: int, address: str, hex_message: str) -> None:
    extra_params: Dict[str, Any] = {"address": address, "message": hex_message, "type": AddressType.XBTC}
    import asyncio
...
@wallet_cmd.command("make_offer", short_help="Create an offer of XBTC/CATs for XBTC/CATs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in XBTC.",
    type=str,
    default="0",
    show_default=True,
