...
from btcgreen.util.ssl_check import DEFAULT_PERMISSIONS_CERT_FILE, DEFAULT_PERMISSIONS_KEY_FILE
...
def get_btcgreen_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "btcgreen_ca.crt")
    key = pkg_resources.resource_string(__name__, "btcgreen_ca.key")
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("btcgreen.net")]),
            critical=False,
        )
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "BTCgreen"),
            x509.NameAttribute(NameOID.COMMON_NAME, "BTCgreen CA"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Organic Farming Division"),
        ]
    )
    root_cert = (
...
def create_all_ssl(
    ...
    *,
    ...
    write_ssl_cert_and_key(btcgreen_ca_crt_path, btcgreen_ca_crt, btcgreen_ca_key_path, btcgreen_ca_key, overwrite=overwrite)
...
def main():
    return make_ca_cert(Path("./btcgreen_ca.crt"), Path("./btcgreen_ca.key"))
