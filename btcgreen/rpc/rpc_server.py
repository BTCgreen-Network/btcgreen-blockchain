...
from btcgreen.rpc.util import wrap_http_handler
from btcgreen.server.outbound_message import NodeType
from btcgreen.server.server import BTCgreenServer, ssl_context_for_client, ssl_context_for_server
...
from btcgreen.util.config import str2bool
...
from btcgreen.util.ws_message import WsRpcMessage, create_payload, create_payload_dict, format_response, pong
...
class RpcServiceProtocol(Protocol):
    ...
    def server(self) -> BTCgreenServer:
        ...
        # a property so as to be read only which allows BTCgreenServer to satisfy
        # Optional[BTCgreenServer]
        ...
    ...
    def get_connections(self, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
        ...
        btcgreen.rpc.rpc_server.default_get_connections()
        """
        ...

    async def on_connect(self, peer: WSBTCgreenConnection) -> None:
...
def default_get_connections(server: BTCgreenServer, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
