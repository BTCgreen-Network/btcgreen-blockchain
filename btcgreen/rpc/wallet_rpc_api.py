...
from btcgreen.data_layer.data_layer_wallet import DataLayerWallet
from btcgreen.pools.pool_wallet import PoolWallet
from btcgreen.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
...
from btcgreen.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
...
from btcgreen.types.blockchain_format.coin import Coin, coin_as_list
...
from btcgreen.util.errors import KeychainIsLocked
from btcgreen.util.ints import uint8, uint32, uint64, uint16
...
from btcgreen.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from btcgreen.wallet.cat_wallet.cat_wallet import CATWallet
...
from btcgreen.wallet.did_wallet.did_wallet_puzzles import match_did_puzzle, program_to_metadata
from btcgreen.wallet.nft_wallet import nft_puzzles
from btcgreen.wallet.nft_wallet.nft_info import NFTInfo, NFTCoinInfo
from btcgreen.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
from btcgreen.wallet.nft_wallet.nft_wallet import NFTWallet
from btcgreen.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from btcgreen.wallet.notification_store import Notification
from btcgreen.wallet.outer_puzzles import AssetType
from btcgreen.wallet.puzzle_drivers import PuzzleInfo, Solver
from btcgreen.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_synthetic_public_key
from btcgreen.wallet.trade_record import TradeRecord
from btcgreen.wallet.trading.offer import Offer
...
from btcgreen.wallet.util.address_type import AddressType, is_valid_address
...
class WalletRpcApi:
    ...
    async def get_latest_coin_spend(
        self, peer: Optional[WSBTCgreenConnection], coin_id: bytes32, latest: bool = True
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xbtc_target_address")
        pool_target = config["pool"].get("xbtc_target_address")
    ...
    async def get_offer_summary(self, request) -> EndpointResult:
        ...
        from btcgreen.util.bech32m import bech32_decode, convertbits
        from btcgreen.wallet.util.puzzle_compression import decompress_object_with_puzzles
    ...
    async def check_offer_validity(self, request) -> EndpointResult:
        offer_hex: str = request["offer"]
        offer = Offer.from_bech32(offer_hex)
        peer: Optional[WSBTCgreenConnection] = self.service.get_full_node_peer()
        if peer is None:
    ...
    async def cancel_offers(self, request: Dict) -> EndpointResult:
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "xbtc")
        ...
        key = None
        if asset_id is not None and asset_id != "xbtc":
    ...
    async def did_get_info(self, request) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.XBTC.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(self, request) -> EndpointResult:
        ...
        xbtc_coin_list = request.get("xbtc_coins", None)
        xbtc_coins = None
        if xbtc_coin_list:
            xbtc_coins = set([Coin.from_json_dict(xbtc_coin) for xbtc_coin in xbtc_coin_list])
        xbtc_change_target = request.get("xbtc_change_target", None)
        if xbtc_change_target is not None:
            if xbtc_change_target[:2] == "xbtc":
                xbtc_change_ph = decode_puzzle_hash(xbtc_change_target)
            else:
                xbtc_change_ph = bytes32(hexstr_to_bytes(xbtc_change_target))
        else:
            xbtc_change_ph = None
        ...
        if mint_from_did:
            sb = await nft_wallet.mint_from_did(
                ...
                xbtc_coins=xbtc_coins,
                xbtc_change_ph=xbtc_change_ph,
            ...
            sb = await nft_wallet.mint_from_xbtc(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
