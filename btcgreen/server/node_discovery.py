...
from btcgreen.protocols import full_node_protocol, introducer_protocol
...
from btcgreen.server.address_manager_sqlite_store import create_address_manager_from_db
from btcgreen.server.outbound_message import NodeType, make_msg, Message
...
class FullNodeDiscovery:
    ...
    def __init__(
        ...
        self.server: BTCgreenServer = server
    ...
    async def update_peer_timestamp_on_message(self, peer: WSBTCgreenConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: WSBTCgreenConnection):
    ...
    async def on_connect_callback(self, peer: WSBTCgreenConnection):
