...
from btcgreen.util.network import is_in_network, is_localhost
...
class BTCgreenServer:
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.btcgreen_ca_crt_path, self.btcgreen_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSBTCgreenConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        try:
            ...
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.btcgreennetwork.org/") as resp:
                    if resp.status == 200:
