...
from btcgreen.protocols.protocol_state_machine import message_requires_reply
from btcgreen.protocols.protocol_timing import API_EXCEPTION_BAN_SECONDS, INVALID_PROTOCOL_BAN_SECONDS
...
from btcgreen.util.api_decorators import get_metadata
from btcgreen.util.errors import Err, ProtocolError
from btcgreen.util.ints import uint16
from btcgreen.util.network import WebServer, is_in_network, is_localhost
...
class BTCgreenServer:
    ...
    all_connections: Dict[bytes32, WSBTCgreenConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        btcgreen_ca_crt_path, btcgreen_ca_key_path = btcgreen_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                btcgreen_ca_crt_path, btcgreen_ca_key_path, public_cert_path, public_key_path
            )
        ...
        if local_type in authenticated_server_types:
            ...
            public_cert_path, public_key_path = public_ssl_paths(root_path, config)
            ssl_context = ssl_context_for_server(
                btcgreen_ca_crt_path, btcgreen_ca_key_path, public_cert_path, public_key_path, log=log
            )
    ...
    async def start_server(self, prefer_ipv6: bool, on_connect: Callable = None):
        if self.webserver is not None:
            raise RuntimeError("BTCgreenServer already started")
    ...
    async def incoming_connection(self, request):
        ...
        try:
            connection = WSBTCgreenConnection.create(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    def connection_closed(self, connection: WSBTCgreenConnection, ban_time: int, closed_connection: bool = False):
