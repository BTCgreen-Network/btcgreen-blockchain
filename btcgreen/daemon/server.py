...
from btcgreen.cmds.init_funcs import check_keys, btcgreen_init, btcgreen_full_version_str
from btcgreen.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from btcgreen.daemon.keychain_server import KeychainServer, keychain_commands
from btcgreen.daemon.windows_signal import kill
from btcgreen.plotters.plotters import get_available_plotters
from btcgreen.plotting.util import add_plot_directory
...
from btcgreen.util.beta_metrics import BetaMetricsLogger
from btcgreen.util.btcgreen_logging import initialize_service_logging
from btcgreen.util.config import load_config
from btcgreen.util.errors import KeychainRequiresMigration, KeychainCurrentPassphraseIsInvalid
...
from btcgreen.util.lock import Lockfile, LockfileError
from btcgreen.util.network import WebServer
...
service_plotter = "btcgreen_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        "btcgreen": "btcgreen",
        "btcgreen_data_layer": "start_data_layer",
        "btcgreen_data_layer_http": "start_data_layer_http",
        ...
        "btcgreen_seeder": "start_seeder",
        "btcgreen_crawler": "start_crawler",
    }
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of BTCgreen will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _btcgreenpos_plotting_command_args(self, request: Any, ignoreCount: bool) -> List[str]:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        plotter: str = request.get("plotter", "btcgreenpos")
        command_args: List[str] = ["btcgreen", "plotters", plotter]

        command_args.extend(self._common_plotting_command_args(request, ignoreCount))

        if plotter == "btcgreenpos":
            command_args.extend(self._btcgreenpos_plotting_command_args(request, ignoreCount))
        elif plotter == "madmax":
    ...
    async def start_plotting(self, request: Dict[str, Any]):
        ...
        plotter = request.get("plotter", "btcgreenpos")
    ...
    async def exit(self) -> None:
        ...
        log.info("btcgreen daemon exiting")
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with BTCGREEN_ROOT: {os.environ['BTCGREEN_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in btcgreen_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    btcgreen_init(root_path, should_check_keys=(not wait_for_unlock))
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"btcgreen-blockchain version: {btcgreen_full_version_str()}")
