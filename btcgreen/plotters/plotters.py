...
from btcgreen.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from btcgreen.plotters.btcgreenpos import get_btcgreenpos_install_info, plot_btcgreen
from btcgreen.plotters.madmax import get_madmax_install_info, plot_madmax
...
def build_parser(subparsers, root_path, option_list, name, plotter_desc):
    ...
    for option in option_list:
        ...
        if option is Options.NUM_BUCKETS:
            u_default = 0 if name == "btcgreenpos" else 256
            parser.add_argument(
                "-u",
                "--buckets",
                type=int,
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in BTCGREEN_ROOT.
    btcgreen_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within BTCGREEN_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
            print(f"Cannot create plotters root path {root_path} {type(e)} {e}.")

    plotters = argparse.ArgumentParser("btcgreen plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, btcgreen_plotter_options, "btcgreenpos", "Create a plot with the default btcgreen plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'btcgreen plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    ...
    if args.plotter is None:
        plotters.print_help()
    elif args.plotter == "btcgreenpos":
        plot_btcgreen(args, btcgreen_root_path)
    elif args.plotter == "madmax":
        plot_madmax(args, btcgreen_root_path, root_path)
    elif args.plotter.startswith("bladebit"):
        plot_bladebit(args, btcgreen_root_path, root_path)
    elif args.plotter == "version":
        show_plotters_version(btcgreen_root_path)
...
def get_available_plotters(root_path) -> Dict[str, Any]:
    ...
    btcgreenpos: Optional[Dict[str, Any]] = get_btcgreenpos_install_info()
    ...
    if btcgreenpos is not None:
        plotters["btcgreenpos"] = btcgreenpos
...
def show_plotters_version(root_path: Path):
    ...
    if "btcgreenpos" in info and "version" in info["btcgreenpos"]:
        print(f"btcgreenpos: {info['btcgreenpos']['version']}")
