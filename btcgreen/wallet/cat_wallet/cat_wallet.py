...
class CATWallet:
    ...
    async def create_tandem_xbtc_tx(
        ...
        if fee > amount_to_claim:
            btcgreen_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            ...
            if first:
                ...
                if need_btcgreen_transaction:
                    if fee > regular_btcgreen_to_claim:
                        ...
                        btcgreen_tx, _ = await self.create_tandem_xbtc_tx(
                            fee, uint64(regular_btcgreen_to_claim), min_coin_amount=min_coin_amount
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
