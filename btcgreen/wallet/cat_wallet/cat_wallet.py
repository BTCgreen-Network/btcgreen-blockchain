...
from btcgreen.wallet.puzzles.genesis_checkers import ALL_LIMITATIONS_PROGRAMS
                ...
                if need_btcgreen_transaction:
                    if fee > regular_btcgreen_to_claim:
                        btcgreen_tx, _ = await self.create_tandem_xbtc_tx(
                            fee, uint64(regular_btcgreen_to_claim), announcement_to_assert=announcement
                        )
                        ...
                        btcgreen_tx, _ = await self.create_tandem_xbtc_tx(fee, uint64(regular_btcgreen_to_claim))
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
