...
from btcgreen.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from btcgreen.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            btcgreen_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        btcgreen_tx = dataclasses.replace(btcgreen_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(btcgreen_tx)
    ...
    async def create_tandem_xbtc_tx(
        ...
        if fee > amount_to_claim:
            btcgreen_coins = await self.standard_wallet.select_coins(
                fee,
                min_coin_amount=min_coin_amount,
                max_coin_amount=max_coin_amount,
                excluded_coin_amounts=exclude_coin_amounts,
            )
            origin_id = list(btcgreen_coins)[0].name()
            ...
            message = None
            for spend in btcgreen_tx.spend_bundle.coin_spends:
            ...
            selected_amount = sum([c.amount for c in btcgreen_coins])
            btcgreen_tx = await self.standard_wallet.generate_signed_transaction(
        ...
        return btcgreen_tx, announcement
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some XBTC as part of this
        regular_btcgreen_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_btcgreen_to_claim = payment_amount

        need_btcgreen_transaction = (fee > 0 or regular_btcgreen_to_claim > 0) and (fee - regular_btcgreen_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_btcgreen_transaction:
                    if fee > regular_btcgreen_to_claim:
                        btcgreen_tx, _ = await self.create_tandem_xbtc_tx(
                            fee,
                            uint64(regular_btcgreen_to_claim),
                    ...
                    elif regular_btcgreen_to_claim > fee:
                        btcgreen_tx, _ = await self.create_tandem_xbtc_tx(
                            fee,
                            uint64(regular_btcgreen_to_claim),
                            min_coin_amount=min_coin_amount,
        ...
        btcgreen_spend_bundle = SpendBundle([], G2Element())
        if btcgreen_tx is not None and btcgreen_tx.spend_bundle is not None:
            btcgreen_spend_bundle = btcgreen_tx.spend_bundle

        return (
            SpendBundle.aggregate(
                [
                    cat_spend_bundle,
                    btcgreen_spend_bundle,
                ]
            ),
            btcgreen_tx,
        )
    ...
    async def generate_signed_transaction(
        ...
        if btcgreen_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=btcgreen_tx.confirmed_at_height,
                    created_at_time=btcgreen_tx.created_at_time,
                    to_puzzle_hash=btcgreen_tx.to_puzzle_hash,
                    amount=btcgreen_tx.amount,
                    fee_amount=btcgreen_tx.fee_amount,
                    confirmed=btcgreen_tx.confirmed,
                    sent=btcgreen_tx.sent,
                    spend_bundle=None,
                    additions=btcgreen_tx.additions,
                    removals=btcgreen_tx.removals,
                    wallet_id=btcgreen_tx.wallet_id,
                    sent_to=btcgreen_tx.sent_to,
                    trade_id=btcgreen_tx.trade_id,
                    type=btcgreen_tx.type,
                    name=btcgreen_tx.name,
                    memos=[],
                )
            )
