...
from btcgreen.consensus.coinbase import farmer_parent_id, pool_parent_id
...
from btcgreen.data_layer.dl_wallet_store import DataLayerStore
from btcgreen.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH, solution_to_pool_state
...
from btcgreen.wallet.cat_wallet.cat_utils import construct_cat_puzzle, match_cat_puzzle
from btcgreen.wallet.cat_wallet.cat_wallet import CATWallet
from btcgreen.wallet.db_wallet.db_wallet_puzzles import MIRROR_PUZZLE_HASH
...
from btcgreen.wallet.did_wallet.did_wallet_puzzles import DID_INNERPUZ_MOD, create_fullpuz, match_did_puzzle
from btcgreen.wallet.key_val_store import KeyValStore
from btcgreen.wallet.nft_wallet.nft_info import NFTWalletInfo
from btcgreen.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs, get_new_owner_did
...
from btcgreen.wallet.notification_manager import NotificationManager
from btcgreen.wallet.outer_puzzles import AssetType
from btcgreen.wallet.puzzle_drivers import PuzzleInfo
from btcgreen.wallet.puzzles.cat_loader import CAT_MOD, CAT_MOD_HASH
...
from btcgreen.wallet.util.address_type import AddressType
from btcgreen.wallet.util.compute_hints import compute_coin_hints
from btcgreen.wallet.util.transaction_type import TransactionType
from btcgreen.wallet.util.wallet_sync_utils import last_change_height_cs, PeerRequestException
...
from btcgreen.wallet.wallet_retry_store import WalletRetryStore
...
class WalletStateManager:
    ...
    async def determine_coin_type(
        self, peer: WSBTCgreenConnection, coin_state: CoinState, fork_height: Optional[uint32]
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        xbtc_spam_amount = self.config.get("xbtc_spam_amount", 1000000)

        # No need to filter anything if the filter is set to 1 or 0 mojos
        if xbtc_spam_amount <= 1:
            return new_coin_state

        spam_filter_after_n_txs = self.config.get("spam_filter_after_n_txs", 200)
        small_unspent_count = await self.coin_store.count_small_unspent(xbtc_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < xbtc_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSBTCgreenConnection) -> Optional[bytes32]:
        # Get minter DID
