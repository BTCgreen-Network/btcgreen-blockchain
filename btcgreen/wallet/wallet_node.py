...
from btcgreen.types.weight_proof import WeightProof
from btcgreen.util.chunks import chunks
from btcgreen.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED, process_config_start_method
from btcgreen.util.errors import KeychainIsLocked, KeychainProxyConnectionFailure, KeychainIsEmpty, KeychainKeyNotFound
...
from btcgreen.util.profiler import mem_profile_task, profile_task
from btcgreen.wallet.transaction_record import TransactionRecord
from btcgreen.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from btcgreen.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
from btcgreen.wallet.util.wallet_sync_utils import (
...
from btcgreen.wallet.wallet_weight_proof_handler import get_wp_fork_point, WalletWeightProofHandler
...
class WalletNode:
    ...
    _server: Optional[BTCgreenServer] = None
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.btcgreen.net"]),
    ...
    def on_disconnect(self, peer: WSBTCgreenConnection):
    ...
    async def long_sync(
        ...
        full_node: WSBTCgreenConnection,
        fork_height: int,
        *,
        rollback: bool,
    ):
    ...
    async def is_peer_synced(
        self, peer: WSBTCgreenConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSBTCgreenConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSBTCgreenConnection]:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSBTCgreenConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            return None
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSBTCgreenConnection]:
        """
        ...
        synced_and_trusted: List[WSBTCgreenConnection] = []
        synced: List[WSBTCgreenConnection] = []
        trusted: List[WSBTCgreenConnection] = []
        neither: List[WSBTCgreenConnection] = []
        all_nodes: List[WSBTCgreenConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peers: List[WSBTCgreenConnection] = self.get_full_node_peers_in_order()
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSBTCgreenConnection):
        if self._wallet_state_manager is None:
    ...
    async def new_peak_from_trusted(self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSBTCgreenConnection):
    ...
    async def new_peak_from_untrusted(
        self, new_peak_hb: HeaderBlock, peer: WSBTCgreenConnection, request_time: uint64
    ) -> bool:
        far_behind: bool = (
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSBTCgreenConnection):
        current_height: uint32 = await self.wallet_state_manager.blockchain.get_finished_sync_up_to()
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSBTCgreenConnection) -> int:
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSBTCgreenConnection, peak: HeaderBlock
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSBTCgreenConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def fetch_puzzle_solution(self, height: uint32, coin: Coin, peer: WSBTCgreenConnection) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSBTCgreenConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSBTCgreenConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
