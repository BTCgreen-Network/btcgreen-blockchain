...
from btcgreen.types.weight_proof import WeightProof, SubEpochData
from btcgreen.util.byte_types import hexstr_to_bytes
from btcgreen.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED
from btcgreen.util.default_root import STANDALONE_ROOT_PATH
from btcgreen.util.ints import uint32, uint64
from btcgreen.util.keychain import KeyringIsLocked, Keychain
from btcgreen.util.path import mkdir, path_from_root
from btcgreen.wallet.util.new_peak_queue import NewPeakQueue, NewPeakQueueTypes, NewPeakItem
...
class WalletNode:
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peer: Optional[WSBTCgreenConnection] = self.get_full_node_peer()
        if peer is None:
    ...
    async def fetch_puzzle_solution(self, peer: WSBTCgreenConnection, height: uint32, coin: Coin) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], fork_height: Optional[uint32] = None, peer: Optional[WSBTCgreenConnection] = None
    ) -> List[CoinState]:
        assert self.server is not None
        all_nodes = self.server.connection_by_type[NodeType.FULL_NODE]
        if len(all_nodes.keys()) == 0:
    ...
    async def fetch_children(
        self, peer: WSBTCgreenConnection, coin_name: bytes32, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
