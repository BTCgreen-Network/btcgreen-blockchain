...
from btcgreen.protocols.wallet_protocol import CoinState
...
from btcgreen.util.condition_tools import conditions_dict_for_solution, pkm_pairs_for_conditions_dict
from btcgreen.util.hash import std_hash
from btcgreen.util.ints import uint8, uint16, uint32, uint64, uint128
...
from btcgreen.wallet.nft_wallet import nft_puzzles
from btcgreen.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from btcgreen.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from btcgreen.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from btcgreen.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
from btcgreen.wallet.payment import Payment
from btcgreen.wallet.puzzle_drivers import PuzzleInfo, Solver
from btcgreen.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
)
from btcgreen.wallet.trading.offer import OFFER_MOD, OFFER_MOD_HASH, NotarizedPayment, Offer
from btcgreen.wallet.transaction_record import TransactionRecord
from btcgreen.wallet.uncurried_puzzle import uncurry_puzzle
from btcgreen.wallet.util.compute_memos import compute_memos
from btcgreen.wallet.util.debug_spend_bundle import disassemble
from btcgreen.wallet.util.transaction_type import TransactionType
from btcgreen.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
...
from btcgreen.wallet.wallet_nft_store import WalletNftStore
...
class NFTWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSBTCgreenConnection) -> None:
    ...
    async def puzzle_solution_received(self, coin_spend: CoinSpend, peer: WSBTCgreenConnection) -> None:
    ...
    async def sign_message(self, message: str, nft: NFTCoinInfo) -> Tuple[G1Element, G2Element]:
        ...
        if uncurried_nft is not None:
            ...
            synthetic_secret_key = calculate_synthetic_secret_key(private, DEFAULT_HIDDEN_PUZZLE_HASH)
            synthetic_pk = synthetic_secret_key.get_g1()
            puzzle: Program = Program.to(("BTCgreen Signed Message", message))
    ...
    async def create_tandem_xbtc_tx(
        self, fee: uint64, announcement_to_assert: Optional[Announcement] = None
    ) -> TransactionRecord:
        btcgreen_coins = await self.standard_wallet.select_coins(fee)
        btcgreen_tx = await self.standard_wallet.generate_signed_transaction(
            uint64(0),
            (await self.standard_wallet.get_new_puzzlehash()),
            fee=fee,
            coins=btcgreen_coins,
            coin_announcements_to_consume={announcement_to_assert} if announcement_to_assert is not None else None,
        )
        assert btcgreen_tx.spend_bundle is not None
        return btcgreen_tx
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, btcgreen_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
            nft_coin=nft_coin,
        ...
        if btcgreen_tx is not None and btcgreen_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, btcgreen_tx.spend_bundle])
            btcgreen_tx = dataclasses.replace(btcgreen_tx, spend_bundle=None)
        ...
        if btcgreen_tx is not None:
            tx_list.append(btcgreen_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            btcgreen_tx = await self.create_tandem_xbtc_tx(fee, Announcement(nft_coin.coin.name(), announcement_to_make))
        else:
        ...
        return nft_spend_bundle, btcgreen_tx
        ...
        for asset, amount in offer_dict.items():
            if amount < 0:
                ...
                if len(offered_coins) == 0:
                    raise ValueError(f"Did not have asset ID {asset.hex() if asset is not None else 'XBTC'} to offer")
    ...
    async def mint_from_did(
        ...
        xbtc_coins: Optional[Set[Coin]] = None,
        xbtc_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an xbtc spend that funds the transactions and pays fees.
        ...
        :param xbtc_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param xbtc_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the xbtc spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an xbtc coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if xbtc_coins is None:
            xbtc_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        assert len(xbtc_coins) > 0
        ...
        # Create the xbtc spend to fund the minting.
        spend_value = sum([coin.amount for coin in xbtc_coins])
        change: uint64 = uint64(spend_value - total_amount)
        xbtc_spends = []
        if xbtc_change_ph is None:
            xbtc_change_ph = await self.standard_wallet.get_new_puzzlehash()
        xbtc_primaries = [
            AmountWithPuzzlehash({"puzzlehash": xbtc_change_ph, "amount": change, "memos": [xbtc_change_ph]})
        ]

        first = True
        for xbtc_coin in xbtc_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(xbtc_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in xbtc_coins]
                message_list.append(
                    Coin(xbtc_coin.name(), xbtc_primaries[0]["puzzlehash"], xbtc_primaries[0]["amount"]).name()
                )
                message: bytes32 = std_hash(b"".join(message_list))

                if len(xbtc_coins) > 1:
                    xbtc_announcement: Optional[Set[bytes]] = {message}
                else:
                    xbtc_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=xbtc_primaries,
                    fee=fee,
                    coin_announcements=xbtc_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(xbtc_coin.name(), message).name()
            ...
            xbtc_spends.append(CoinSpend(xbtc_coin, puzzle, solution))
        xbtc_spend = await self.standard_wallet.sign_transaction(xbtc_spends)
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, xbtc_spend, *eve_spends])
        return total_spend

    async def mint_from_xbtc(
        self,
        metadata_list: List[Dict[str, Any]],
        target_list: Optional[List[bytes32]] = [],
        mint_number_start: Optional[int] = 1,
        ...
        """
        Minting NFTs from a single XBTC spend using intermediate launcher puzzle
        :param metadata_list: A list of dicts containing the metadata for each NFT to be minted
        :param target_list: [Optional] a list of targets for transfering minted NFTs (aka airdrop)
        :param mint_number_start: [Optional] The starting point for mint number used in intermediate launcher
        puzzle. Default: 1
        ...
        funding_coin = xbtc_coins.copy().pop()
        ...
        # Empty set to load with the announcements we will assert from XBTC to
        # match the announcements from the intermediate launcher puzzle
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # create an ASSERT_COIN_ANNOUNCEMENT for the XBTC spend. The
            # intermediate launcher coin issues a CREATE_COIN_ANNOUNCEMENT of
            # the mint_number and mint_total for the launcher coin it creates
            intermediate_announcement_message = std_hash(int_to_bytes(mint_number) + int_to_bytes(mint_total))
            coin_announcements.add(std_hash(intermediate_launcher_coin.name() + intermediate_announcement_message))

            # Create the launcher coin, and add its id to a list to be asserted in the XBTC spend
            ...
            # Annnouncements for eve spend. These are asserted by the xbtc spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        for xbtc_coin in xbtc_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=xbtc_primaries + primaries,
                    fee=fee,
                    coin_announcements=xbtc_announcement if len(xbtc_coins) > 1 else None,
...
if TYPE_CHECKING:
    from btcgreen.wallet.wallet_protocol import WalletProtocol
