...
from btcgreen.types.blockchain_format.coin import hash_coin_ids, Coin
...
from btcgreen.util.merkle_set import confirm_not_included_already_hashed, confirm_included_already_hashed, MerkleSet
from btcgreen.wallet.util.peer_request_cache import PeerRequestCache
...
async def fetch_last_tx_from_peer(height: uint32, peer: WSBTCgreenConnection) -> Optional[HeaderBlock]:
...
async def request_and_validate_removals(
    peer: WSBTCgreenConnection, height: uint32, header_hash: bytes32, coin_name: bytes32, removals_root: bytes32
) -> bool:
...
async def request_header_blocks(
    peer: WSBTCgreenConnection, start_height: uint32, end_height: uint32
...
async def _fetch_header_blocks_inner(
    all_peers: List[Tuple[WSBTCgreenConnection, bool]],
