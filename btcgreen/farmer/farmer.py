...
from btcgreen.pools.pool_config import PoolWalletConfig, load_pool_config, add_auth_key
...
from btcgreen.util.config import load_config, save_config, config_path_for_filename, get_config_lock
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        ...
        if "xbtc_target_address" not in self.config:
            self.config = config["farmer"]
        if "xbtc_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "xbtc_target_address" not in self.config or "xbtc_target_address" not in self.pool_config:
            log.debug("xbtc_target_address missing in the config")
            return False
