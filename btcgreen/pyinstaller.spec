...
def solve_name_collision_problem(analysis):
    ...
    need to create the `btcgreen` directory, since the names collide.
...
if os.path.exists(f"{ROOT}/madmax/btcgreen_plot"):
    binaries.extend([
        (
...
if os.path.exists(f"{ROOT}/madmax/btcgreen_plot_k34",):
    binaries.extend([
        (
...
add_binary("start_data_layer_http", f"{ROOT}/btcgreen/data_layer/data_layer_server.py", COLLECT_ARGS)
