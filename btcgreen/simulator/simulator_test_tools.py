...
from btcgreen.daemon.server import WebSocketServer, daemon_launch_lock_path
...
from btcgreen.simulator.ssl_certs import (
...
from btcgreen.simulator.start_simulator import async_main as start_simulator_main
from btcgreen.ssl.create_ssl import create_all_ssl
from btcgreen.types.blockchain_format.sized_bytes import bytes32
from btcgreen.util.bech32m import encode_puzzle_hash
from btcgreen.util.config import create_default_btcgreen_config, load_config, save_config
from btcgreen.util.errors import KeychainFingerprintExists
from btcgreen.util.ints import uint32
from btcgreen.util.keychain import Keychain
from btcgreen.util.lock import Lockfile
...
def create_config(
    ...
    # create btcgreen directories
    create_default_btcgreen_config(btcgreen_root)
    create_all_ssl(
        btcgreen_root,
    ...
    # load config
    config = load_config(btcgreen_root, "config.yaml")
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(keychain, fingerprint), "txbtc")
    config["simulator"]["plot_directory"] = "test-simulator/plots"
    # save config
    save_config(btcgreen_root, "config.yaml", config)
    return config


async def start_simulator(btcgreen_root: Path, automated_testing: bool = False) -> AsyncGenerator[FullNodeSimulator, None]:
    sys.argv = [sys.argv[0]]  # clear sys.argv to avoid issues with config.yaml
    service = await start_simulator_main(True, automated_testing, root_path=btcgreen_root)
    await service.start()
...
async def get_full_btcgreen_simulator(
    btcgreen_root: Path,
    ...
    """
    A btcgreen root Path is required.
    The btcgreen root Path can be a temporary directory (tempfile.TemporaryDirectory)
    ...
    This test is fully interdependent and can be used without the rest of the btcgreen test suite.
    ...
    with Lockfile.create(daemon_launch_lock_path(btcgreen_root)):
        ...
        crt_path = btcgreen_root / config["daemon_ssl"]["private_crt"]
        key_path = btcgreen_root / config["daemon_ssl"]["private_key"]
        ca_crt_path = btcgreen_root / config["private_ssl_ca"]["crt"]
        ca_key_path = btcgreen_root / config["private_ssl_ca"]["key"]

        shutdown_event = asyncio.Event()
        ws_server = WebSocketServer(btcgreen_root, ca_crt_path, ca_key_path, crt_path, key_path, shutdown_event)
        await ws_server.setup_process_global_state()
        await ws_server.start()

        async for simulator in start_simulator(btcgreen_root, automated_testing):
            yield simulator, btcgreen_root, config, mnemonic, fingerprint, keychain
