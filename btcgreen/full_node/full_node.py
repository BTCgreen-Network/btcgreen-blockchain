...
class FullNode:
    ...
    def remove_subscriptions(self, peer: ws.WSBTCgreenConnection):
    ...
    async def sync_from_fork_point(
        ...
        async def fetch_block_batches(batch_queue: asyncio.Queue, peers_with_peak: List[ws.WSBTCgreenConnection]):
            try:
        ...
        batch_queue: asyncio.Queue[Tuple[ws.WSBTCgreenConnection, List[FullBlock]]] = asyncio.Queue(maxsize=buffer_size)
    ...
    async def update_wallets(
        ...
        for peer, changes in changes_for_peer.items():
            if peer not in self.server.all_connections:
                continue
            ws_peer: ws.WSBTCgreenConnection = self.server.all_connections[peer]
...
async def node_next_block_check(
    peer: ws.WSBTCgreenConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
