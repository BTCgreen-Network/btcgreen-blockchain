...
def check_legacy_keyring_keys_present(keyring: LegacyKeyring) -> bool:
    ...
    from btcgreen.util.keychain import MAX_KEYS, default_keychain_service, default_keychain_user, get_private_key_user
...
class KeyringWrapper:
    ...
    def __init__(self, keys_root_path: Path = DEFAULT_KEYS_ROOT_PATH, force_legacy: bool = False):
        ...
        """
        from btcgreen.util.errors import KeychainNotSet
    ...
    def set_master_passphrase(
        ...
        *,
        ...
        """
        from btcgreen.util.errors import KeychainCurrentPassphraseIsInvalid, KeychainRequiresMigration
        from btcgreen.util.keychain import supports_os_passphrase_storage
    ...
    def confirm_migration(self) -> bool:
        ...
        if master_passphrase == DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE:
            ...
            print(
                "Would you like to set a master passphrase now? Use 'btcgreen passphrase set' to change the passphrase.\n"
            )

            response = prompt_yes_no("Set keyring master passphrase?")
            if response:
                from btcgreen.cmds.passphrase_funcs import prompt_for_new_passphrase
                ...
                print(
                    "Will skip setting a master passphrase. Use 'btcgreen passphrase set' to set the master passphrase.\n"
                )
        else:
            import colorama
    ...
    def migrate_legacy_keys(self) -> MigrationResults:
        from btcgreen.util.keychain import MAX_KEYS, Keychain, get_private_key_user
    ...
    async def migrate_legacy_keyring_interactive(self) -> bool:
        ...
        """
        from btcgreen.cmds.passphrase_funcs import async_update_daemon_migration_completed_if_running

        # Let the user know about the migration.
        if not self.confirm_migration():
            print("Migration aborted, can't run any btcgreen commands.")
            return False
