...
DEFAULT_USER = f"user-btcgreen-{CURRENT_KEY_VERSION}"  # e.g. user-btcgreen-1.8
DEFAULT_SERVICE = f"btcgreen-{DEFAULT_USER}"  # e.g. btcgreen-user-btcgreen-1.8
...
def supports_keyring_passphrase() -> bool:
    # Support can be disabled by setting BTCGREEN_PASSPHRASE_SUPPORT to 0/false
    return os.environ.get("BTCGREEN_PASSPHRASE_SUPPORT", "true").lower() in ["1", "true"]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from btcgreen.cmds.passphrase_funcs import prompt_for_passphrase
