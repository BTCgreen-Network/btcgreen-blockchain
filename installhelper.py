...
# Gets the version from the result of "btcgreen version"
...
def get_btcgreen_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["btcgreen", "version"], capture_output=True)
...
def update_version(package_json_path: str):
    ...
    data["version"] = get_btcgreen_version()
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/btcgreen-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/btcgreen-blockchain-gui/packages/gui/package.json")
