...
--appVersion=$BTCGREEN_INSTALLER_VERSION --executable-name=btcgreen-blockchain
LAST_EXIT_CODE=$?
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	...
	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/BTCgreen-Network/btcgreen-blockchain/issues/3846
  ...
  --license ../LICENSE --options.bin btcgreen-blockchain --options.name btcgreen-blockchain
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
