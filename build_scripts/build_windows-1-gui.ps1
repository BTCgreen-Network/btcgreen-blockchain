...
git submodule update --init btcgreen-blockchain-gui
...
# Other than `btcgreen-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, unused packages should be removed.
Write-Output "Remove unused @btcgreen packages to make cache slim"
...
# Remove `packages/gui/node_modules/@btcgreen` because it causes an error on later `electron-packager` command
Remove-Item "@btcgreen" -Recurse -Force
