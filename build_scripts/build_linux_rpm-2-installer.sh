...
# Builds CLI only rpm
CLI_RPM_BASE="btcgreen-blockchain-cli-$BTCGREEN_INSTALLER_VERSION-1.$REDHAT_PLATFORM"
mkdir -p "dist/$CLI_RPM_BASE/opt/btcgreen"
mkdir -p "dist/$CLI_RPM_BASE/usr/bin"
cp -r dist/daemon/* "dist/$CLI_RPM_BASE/opt/btcgreen/"
ln -s ../../opt/btcgreen/btcgreen "dist/$CLI_RPM_BASE/usr/bin/btcgreen"
...
fpm -s dir -t rpm \
  ...
  --name btcgreen-blockchain-cli \
  --license Apache-2.0 \
  --version "$BTCGREEN_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "BTCgreen is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
  --depends /usr/lib64/libcrypt.so.1 \
  .
# CLI only rpm done

cp -r dist/daemon ../btcgreen-blockchain-gui/packages/gui

# Change to the gui package
cd ../btcgreen-blockchain-gui/packages/gui || exit 1

# sets the version for btcgreen-blockchain in package.json
cp package.json package.json.orig
jq --arg VER "$BTCGREEN_INSTALLER_VERSION" '.version=$VER' package.json > temp.json && mv temp.json package.json
...
PRODUCT_NAME="btcgreen"
echo electron-builder build --linux rpm "${OPT_ARCH}" \
  --config.extraMetadata.name=btcgreen-blockchain \
  --config.productName="${PRODUCT_NAME}" --config.linux.desktop.Name="BTCgreen Blockchain" \
  --config.rpm.packageName="btcgreen-blockchain"
...
GUI_RPM_NAME="btcgreen-blockchain-${BTCGREEN_INSTALLER_VERSION}-1.${REDHAT_PLATFORM}.rpm"
mv "dist/${PRODUCT_NAME}-${BTCGREEN_INSTALLER_VERSION}.rpm" "../../../build_scripts/dist/${GUI_RPM_NAME}"
